...
class SettingsController {
  async getWebhooks(req, res) {
    try {
      const [webhooks] = await db.query('SELECT * FROM discord_webhooks ORDER BY created_at DESC');
      res.json(webhooks);
    } catch (error) {
  ...
  async saveWebhook(req, res) {
    try {
      ...
      await db.query('INSERT INTO discord_webhooks (url) VALUES (?)', [url]);
  ...
  async updateWebhook(req, res) {
    try {
      ...
      await db.query('UPDATE discord_webhooks SET url = ? WHERE id = ?', [url, id]);
